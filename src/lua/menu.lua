...
-- user options
local o = {
    use_mpv_impl = false,    -- use the Qt menu plugin by default on Linux
...
local function parse_input_conf(conf)
    ...
    for line in conf:gmatch('[^\r\n]+') do
        ...
        -- dialog.lua used to be a separate script, so existing input.conf
        -- files address a client named "dialog". Dialog handling now lives in
        -- this script and its client name can vary (for example menu2 when the
        -- C plugin is also named menu), so redirect messages to the actual
        -- client. Do not broadcast generic messages such as "open", because
        -- mpv's built-in console also handles a message with that name.
        if cmd then
            cmd = cmd:gsub('^(%s*)script%-message%-to%s+dialog%s+',
                           '%1script-message-to ' .. mp.get_script_name() .. ' ')
        end
...
if use_mpv_impl then
    ...
    mp.add_forced_key_binding('MBTN_RIGHT', 'show-mpv-context-menu', function()
        msg.info('right click: requesting mpv internal context menu')
        mp.commandv('context-menu')
    end)
else
    mp.add_forced_key_binding('MBTN_RIGHT', 'show', function()
        msg.info('right click: requesting native Qt menu')
        mp.commandv('script-message', 'native-menu/show')
    end)
end
...
-- Native dialogs and clipboard integration
local opts = require('mp.options')
local utils = require('mp.utils')
local msg = require('mp.msg')
...
local function save_cb(path)
    if save_action == 'screenshot' then
        mp.commandv('screenshot-to-file', path, save_arg1)
...
mp.register_script_message('open', function(action)
    ...
    mp.commandv('script-message', 'dialog/open-multi', mp.get_script_name())
end)

-- open folder dialog
mp.register_script_message('open-folder', function()
    mp.commandv('script-message', 'dialog/open-folder', mp.get_script_name())
end)
...
mp.register_script_message('save', function(action, arg1)
    ...
    mp.commandv('script-message', 'dialog/save', mp.get_script_name())
end)

-- open clipboard
mp.register_script_message('open-clipboard', function(action)
    open_action = action
    mp.commandv('script-message', 'clipboard/get', mp.get_script_name())
end)
...
mp.register_script_message('set-clipboard', function(text)
    ...
    mp.commandv('script-message', 'clipboard/set', value)
end)
